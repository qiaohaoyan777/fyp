...
public class TestEmailController {
    ...
    // 默认测试邮箱，可以在 application.yml 中通过 test.email 覆盖
    ...
    public BaseResponse<String> testConnection() {
        try {
            ...
            if (connected) {
                ...
                return ResultUtils.error(ErrorCode.EMAIL_CONNECT_ERROR, "Gmail SMTP connection failed, please check logs.");
            }
        } catch (Exception e) {
    ...
    public BaseResponse<String> sendTestEmail(
        ...
        // 优化：增加了对空字符串的校验，防止传入空的 email 参数导致发送失败
        String targetEmail = (email != null && !email.trim().isEmpty()) ? email : testEmail;
