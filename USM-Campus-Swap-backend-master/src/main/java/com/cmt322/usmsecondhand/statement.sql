...
(
    id          BIGINT AUTO_INCREMENT COMMENT 'Primary Key'
        PRIMARY KEY,
    userId      BIGINT                              NOT NULL COMMENT 'ID of the user who bookmarked the item',
    goodsId     BIGINT                              NOT NULL COMMENT 'ID of the bookmarked item',
    createTime  TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT 'Time when the item was added to wishlist',
    CONSTRAINT uk_user_goods
        UNIQUE (userId, goodsId) COMMENT 'Prevent duplicate bookmarks for the same item by the same user'
)
    COMMENT 'Product Wishlist/Bookmark Table';
